...
const CustomerForm = ({ reset, fields, customerType, ...props }: CustomerFormProps) => {
  ...
  const {
    ...
    resource: `customers/${customerType}`,
  ...
  useEffect(() => {
    if (value?.isAdd) {
      reset(getInputCustomer({}, 'bill-customers'))
    } else {
      reset(getInputCustomer(value, 'bill-customers'))
    }
  }, [value])
