...
const APIKeyPanel: React.FC<APIKeyPanelProps> = () => {
    ...
    const apiDataKeys = [
        ...
        { key: "created", label: "Created at", value: getReadableDate(apiKey?.created) },
        {
            key: "revoked",
            label: "Revoked",
    ...
    return (
        <>
            ...
            <div className="pb-6">
                {apiData?.data ? (
                    apiDataKeys.map((adk) => (
                        <div key={adk.key} className="text-[#252626] text-base grid grid-cols-3 border-b border-[#d5dce3] py-1.5 px-12 items-center justify-center">
                            <div className="font-medium">{adk.label}</div>
                            <div className="col-span-2">{adk.value}</div>
                        </div>
                    ))
                ) : (
                    <div className="flex items-center justify-center text-[#848fa7] text-lg">
                        There is no API Key
                    </div>
                )}
            </div>
