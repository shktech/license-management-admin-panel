      ...
      <ul className="mb-3 mt-3 flex flex-col gap-2.5 pl-6">
        {item.map((item: any, index: number) => (
          <li key={index}>
            <Link
              href={item.route}
              className={`group relative flex items-center gap-2.5 rounded-md px-4 py-1.5 text-bodydark2 duration-300 ease-in-out ${pathname === item.route ? "text-new-blue-2" : ""
                }`}
            >
              ...
              <span className={`${pathname === item.route ? 'font-medium' : ''}`}>{item.label}</span>
            </Link>
          </li>
        ))}
      </ul>
